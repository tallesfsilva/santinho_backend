...
export async function globalErrorHandler(
  ...
  if (err instanceof CelebrateError) {
    logger.error(
      `${400} - ${err.details.values().next().value.details[0].message} - ${request.originalUrl
    ...
    let messageString;
    const details = err.details.values().next().value.details[0];
      ...
      switch (type) {
        case 'any.only':
          messageString = context.valids[0].path
            ? `O campo ${context.label} deve ter o mesmo valor do campo ${valids[0]}.`
            : `O campo ${context.label} pode ter o(s) valor(es):${valids}`;
          break;
        case 'any.required':
